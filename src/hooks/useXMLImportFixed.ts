...
export function useXMLImport() {
  ...
  // Verificação de duplicatas - USANDO ESTRUTURA CORRETA
  const verificarDuplicata = async (xmlData: XMLData): Promise<{ isDuplicate: boolean; reason: string }> => {
    try {
      // Critério 1: Verificar por número da NFe na descrição
      ...
      // Critério 2: Verificar por chave de acesso
  ...
  // Buscar ou criar fornecedor - USANDO ESTRUTURA CORRETA
  ...
  const buscarDadosPadrao = async () => {
    try {
      ...
      const { data: filiais, error: filError } = await supabase
        .from('filiais')
        .select('id')
        .limit(1);
      ...
      if (catError) console.error('Erro ao buscar categoria:', catError);
      if (filError) console.error('Erro ao buscar filial:', filError);

      return { categoriaId, filialId };
    } catch (error) {
  ...
  // Criar contas a pagar - USANDO ESTRUTURA CORRETA
  const criarContasPagar = async (xmlData: XMLData, fornecedorId: number | null): Promise<boolean> => {
    try {
      ...
      if (!contaPrincipal || !contaPrincipal.id) {
        console.error('ID da conta principal não retornado');
        return false;
      }

      console.log(`Conta principal criada: ${contaPrincipal.id}`);
      ...
      for (let i = 0; i < xmlData.duplicatas.length; i++) {
        ...
        const { error: parcelaError } = await supabase
          ...
          .insert({
            ...
            pago: false,
            observacao: `Parcela ${i + 1} de ${xmlData.duplicatas.length} - Importado de XML`
          });
  ...
  const processFile = async (file: File): Promise<ProcessResult> => {
    try {
      ...
      // Continuar mesmo se não conseguir criar fornecedor

      // 4. Criar contas a pagar
      ...
      return {
        success: true,
        message: `✅ NFe ${xmlData.numeroNFe || 'sem número'} importada com sucesso - R$ ${xmlData.valorTotal.toFixed(2)} (${xmlData.duplicatas.length} ${xmlData.duplicatas.length === 1 ? 'parcela' : 'parcelas'})`,
        fileName: file.name,
        xmlData
      };
