...
export function useXMLImport() {
  ...
  // Buscar ou criar fornecedor - USANDO APENAS CAMPOS QUE EXISTEM
  const buscarOuCriarFornecedor = async (xmlData: XMLData): Promise<number | null> => {
    try {
      ...
      // Criar novo fornecedor - APENAS CAMPOS QUE EXISTEM
  ...
  // Criar conta APENAS na tabela principal - USANDO APENAS CAMPOS QUE EXISTEM
  const criarContaPrincipal = async (xmlData: XMLData, fornecedorId: number | null): Promise<boolean> => {
    try {
      ...
      // Criar APENAS a conta principal - USANDO APENAS CAMPOS QUE EXISTEM
      const { error: contaError } = await supabase
        ...
        .insert({
          ...
          // REMOVIDO: data_vencimento (campo não existe)
          // REMOVIDO: observacoes (campo não existe)
        });
